...
int n;
int dp[2501];
int rec(int i,vector<int> &a){  //O(n^2)
     //best lis at ending level.
     //pruning
     if(i<0){return 0;}
...
void solve(){
    ...
    // int best=0;
    // for(int i=0;i<n;i++){
    //     best=max(best,rec(i,a));
    // }
    // cout<<best;

    vector<int> lis;int insertedat[n];
    for(int i=0;i<n;i++){
    ...
    // for(int i=0;i<n;i++){cout<<insertedat[i]<<" ";}
    int l=lis.size();
    int final_lis[l];
    int curlen=l;
    for(int i=n-1;i>=0;i--){
        if(insertedat[i]==curlen){
            final_lis[curlen-1]=a[i];curlen--;
        }
    }
    cout<<l<<nl;
    for(auto i:final_lis){cout<<i<<" ";}
    cn;
